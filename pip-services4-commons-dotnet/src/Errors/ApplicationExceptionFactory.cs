    ...
    public static class ApplicationExceptionFactory
    {
        ...
        public static ApplicationException Create(ErrorDescription description)
        {
            ...
            if (description.Category == ErrorCategory.Unknown)
                ...
                error = new InternalException(description.TraceId, description.Code, description.Message);
            else if (description.Category == ErrorCategory.Misconfiguration)
                error = new ConfigException(description.TraceId, description.Code, description.Message);
            else if (description.Category == ErrorCategory.NoResponse)
                error = new ConnectionException(description.TraceId, description.Code, description.Message);
            else if (description.Category == ErrorCategory.FailedInvocation)
                error = new InvocationException(description.TraceId, description.Code, description.Message);
            else if (description.Category == ErrorCategory.NoFileAccess)
                error = new FileException(description.TraceId, description.Code, description.Message);
            else if (description.Category == ErrorCategory.BadRequest)
                error = new BadRequestException(description.TraceId, description.Code, description.Message);
            else if (description.Category == ErrorCategory.Unauthorized)
                error = new UnauthorizedException(description.TraceId, description.Code, description.Message);
            else if (description.Category == ErrorCategory.Conflict)
                error = new ConflictException(description.TraceId, description.Code, description.Message);
