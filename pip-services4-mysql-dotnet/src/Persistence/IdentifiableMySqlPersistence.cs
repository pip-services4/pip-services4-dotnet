...
namespace PipServices4.Mysql.Persistence
{
    ...
    /// </code>
    /// </example>
    public class IdentifiableMySqlPersistence<T, K> : MySqlPersistence<T>, IWriter<T, K>, IGetter<T, K>, ISetter<T>, IPartialUpdater<T, K>
        ...
        where K : class
    {
        ...
        public IdentifiableMySqlPersistence(string tableName)
            : base(tableName)
        { }
        ...
        public virtual async Task<T> SetAsync(IContext context, T item)
        {
            ...
            query += "; SELECT * FROM " + QuoteIdentifier(_tableName) + " WHERE id=@Param" + values.Count;

            AnyValueMap result = (await ExecuteReaderAsync(query, values)).FirstOrDefault();
        ...
        public virtual async Task<T> UpdateAsync(IContext context, T item)
        {
            ...
            var query = "UPDATE " + QuoteIdentifier(_tableName) + " SET " + @params + " WHERE id=@Param" + values.Count;
            query += "; SELECT * FROM " + QuoteIdentifier(_tableName) + " WHERE id=@Param" + values.Count;
        ...
        public virtual async Task<T> DeleteByIdAsync(IContext context, K id)
        {
            ...
            var query = "SELECT * FROM " + QuoteIdentifier(_tableName) + " WHERE id=@Param1";
            query += "; DELETE FROM " + QuoteIdentifier(_tableName) + " WHERE id=@Param1";
