...
namespace PipServices4.MySql.Persistence
{
    ...
    /// </code>
    /// </example>
    public class MySqlPersistence<T> : IReferenceable, IUnreferenceable, IReconfigurable, IOpenable, ICleanable where T : new()
    {
        ...
        /// </summary>
        protected MySqlData.MySqlClient.MySqlConnection _client;
        ...
        public virtual void SetReferences(IReferences references)
        {
            ...
            // Get connection
            _connection = _dependencyResolver.GetOneOptional("connection") as MySqlConnection;
        ...
        public virtual async Task CloseAsync(IContext context)
        {
            if (IsOpen())
            {
                if (_connection == null)
                    throw new InvalidStateException(context != null ? ContextResolver.GetTraceId(context) : null, 
                        "NO_CONNECTION", "MySql connection is missing");
                ...
                PagingParams paging = null, string sort = null, string select = null)
        {
            ...
            var total = pagingEnabled ? (long?)await GetCountByFilterAsync(context, filter) : null;
        ...
        public virtual async Task<T> CreateAsync(IContext context, T item)
        {
            ...
            var query = "INSERT INTO " + QuoteIdentifier(_tableName) + " (" + columns + ") VALUES (" + @params + ")";

            await ExecuteNonQuery(query, map);
            var newItem = item;
        ...
        {
            var result = await ExecuteReaderAsync("SHOW TABLES LIKE '" + tableName + "'");
            return result != null && result.Count > 0;
        }

        protected virtual void SetParameters(MySqlData.MySqlClient.MySqlCommand cmd, IEnumerable<object> values)
        {
        ...
        protected virtual void AddParameter(MySqlData.MySqlClient.MySqlCommand cmd, string name, object value)
        {
        ...
        {
            using (var cmd = new MySqlData.MySqlClient.MySqlCommand(cmdText, _client))
            {
        ...
        protected static async Task<List<AnyValueMap>> ExecuteReaderAsync(MySqlData.MySqlClient.MySqlCommand cmd)
        {
            using (var reader = await cmd.ExecuteReaderAsync())
            {
                ...
                {
                    var dataTypeName = row["DataType"].ToString();
                    if (row.Table.Columns.Contains("DataTypeName"))
                        dataTypeName = row["DataTypeName"].ToString();
