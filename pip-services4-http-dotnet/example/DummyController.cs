...
namespace PipServices4.Http
{
    public sealed class DummyController : IDummyController, ICommandable
    {
        ...
        public async Task<string> CheckTraceId(IContext context)
        {
            return await Task.FromResult(ContextResolver.GetTraceId(context));
        }

        public Task RaiseExceptionAsync(IContext context)
        {
            throw new NotFoundException(ContextResolver.GetTraceId(context), "TEST_ERROR", "Dummy error in controller!");
        }
